...
############# read netCDF file ###############
fn = r'./netCDF/EMIT_L2A_RFLUNCERT_001_20230119T114235_2301907_004.nc'
fn_nc = Dataset(fn)
...
##### summary #####
print("E is:")
summary_E = fn_nc.variables['reflectance_uncertainty']
print(summary_E)

##### attributes #####
print("E's attributes are:")
attributes_E = fn_nc.variables['reflectance_uncertainty'].ncattrs()
print(attributes_E)

##### units #####
print("E's units are:")
units_E = fn_nc.variables['reflectance_uncertainty'].units
print(units_E)

##### FillValue #####
print("E's _FillValue are:")
FillValue_E = fn_nc.variables['reflectance_uncertainty']._FillValue
print(FillValue_E)

##### standard_name #####
print("E's long_name are:")
standard_name_E = fn_nc.variables['reflectance_uncertainty'].long_name
print(standard_name_E)

############# get E variable ###############
E = fn_nc.variables['reflectance_uncertainty']
print(E.shape)
...
############# get downtrack, crosstrack and bands ###############
downtrack = E[0]
print(len(downtrack))

############# get lat variable and lat_min, lat_max ###############
lat = fn_nc.groups['location']['lat']
lat_min = np.min(lat)
lat_max = np.max(lat)

# get lon variable and lon_min, lon_max ###############a
lon = fn_nc.groups['location']['lon']
lon_min = np.min(lon)
lon_max = np.max(lon)
...
m = Basemap(projection='cyl', llcrnrlon=lon_min, urcrnrlon=lon_max,
            llcrnrlat=lat_min, urcrnrlat=lat_max, resolution='l')
...
m.drawparallels(np.arange(-90, 90, 30),
                labels=[1, 0, 0, 0], fontsize=13, linewidth=0.8)
m.drawmeridians(np.arange(-180, 180, 45),
                labels=[0, 0, 0, 1], fontsize=13, linewidth=0.8)
...
##### save the figure #####
plt.savefig(r'./evaporation.tif', dpi=100)
